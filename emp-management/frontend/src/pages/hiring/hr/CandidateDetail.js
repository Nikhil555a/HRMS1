...
//       {/* Avatar Card */}
//       <div className="card" style={{ textAlign: 'center', padding: '24px' }}>
//         <div className="av" style={{ width: 72, height: 72, background: avColor(cand.name), color: 'white', fontSize: '28px', margin: '0 auto 12px', fontFamily: 'var(--font-display)', fontWeight: 800 }}>
//           {initials(cand.name)}
//         </div>
//         <h2 style={{ fontSize: '17px', fontWeight: 700, fontFamily: 'var(--font-display)' }}>{cand.name}</h2>
//         <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginTop: '2px' }}>{cand.email}</p>
//         <p style={{ color: 'var(--text-muted)', fontSize: '13px' }}>{cand.phone}</p>
//         {cand.resume?.url && (
//           <a href={cand.resume.url} target="_blank" rel="noreferrer"
//             className="btn btn-ghost btn-sm" style={{ marginTop: '14px', display: 'inline-flex' }}>
//             📄 View Resume
//           </a>
//         )}
//       </div>

//       {/* ── Job Details Card — FIXED ── */}
//       <div className="card">
//         <div className="card-title" style={{ marginBottom: '10px' }}>Job Details</div>
//         {[
//           ['Position',    cand.job?.title || cand.jobTitle],
//           ['Dept',        cand.job?.department],
//           ['Source',      cand.source?.platform],
//           ...(cand.source?.referredBy ? [['Referred By', cand.source.referredBy]] : []),
//           ['Assigned To', cand.assignedTo?.name],
//         ].map(([label, value]) => (
//           <div key={label} style={{
//             display: 'flex',
...
//             gap: '16px',
//           }}>
//             <span style={{ fontSize: '13px', color: 'var(--text-muted)', flexShrink: 0, minWidth: '80px' }}>
//               {label}
//             </span>
//             <span style={{ fontSize: '13px', fontWeight: 600, textAlign: 'right', wordBreak: 'break-word' }}>
//               {value || '—'}
//             </span>
//           </div>
//         ))}
...
//       {/* ── Professional Info Card — FIXED ── */}
//       <div className="card">
//         <div className="card-title" style={{ marginBottom: '12px' }}>Professional Info</div>
//         {[
//           ['Current Company', cand.currentCompany],
//           ['Designation',     cand.currentDesignation],
//           ['Total Exp',       cand.totalExperience],
//           ['Relevant Exp',    cand.relevantExperience],
//           ['Current CTC',     fmtMoney(cand.currentCTC)],
//           ['Expected CTC',    fmtMoney(cand.expectedCTC)],
...
//             gap: '16px',
//           }}>
//             <span style={{ fontSize: '13px', color: 'var(--text-muted)', flexShrink: 0, minWidth: '110px' }}>
//               {label}
//             </span>
//             <span style={{ fontSize: '13px', fontWeight: 600, textAlign: 'right', wordBreak: 'break-word' }}>
//               {value || '—'}
...
//             {cand.skills.map(s => <span key={s} className="skill-pill">{s}</span>)}
//           </div>
//         </div>
//       )}
...
//           }}>
//             <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2" style={{ flexShrink: 0, marginTop: '1px' }}>
...
//           <button
//             type="button"
//             onClick={() => { if (reason.trim()) { onConfirm(reason); setReason(''); } else toast.error('Reason required'); }}
//             style={{ background: '#ef4444', color: 'white', border: 'none', padding: '8px 18px', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '13px' }}
//           >
//             Confirm Reject
//           </button>
//         </div>
...
//               background: danger ? '#ef444415' : '#10b98115',
//               border: `0.5px solid ${danger ? '#ef444430' : '#10b98130'}`,
//               display: 'flex', alignItems: 'center', justifyContent: 'center',
//             }}>
//               <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={danger ? '#ef4444' : '#10b981'} strokeWidth="2">
//                 <circle cx="12" cy="12" r="10"/>
//                 <line x1="12" y1="8" x2="12" y2="12"/>
//                 <line x1="12" y1="16" x2="12.01" y2="16"/>
//               </svg>
...
//   const [reasonModal, setReasonModal] = useState({ open: false, title: '', onConfirm: null });
//   const closeReason = () => setReasonModal(r => ({ ...r, open: false }));
...
//   React.useEffect(() => { fetchCand(); }, [fetchCand]);
...
//   /* ── Offer status — Accepted uses ConfirmModal, Rejected uses ReasonModal ── */
//   const updateOfferStatus = (status) => {
//     if (status === 'Accepted') {
//       setConfirm({
//         open: true,
//         title: 'Accept Offer?',
//         message: <span>Mark this offer as <strong>Accepted</strong>? This will update the candidate's stage accordingly.</span>,
//         danger: false,
//         onConfirm: async () => {
//           closeConfirm();
//           try {
//             await api.patch(`/hiring/documents/${id}/offer-status`, { status: 'Accepted', rejectionReason: '' });
//             toast.success('Offer accepted!'); fetchCand();
//           } catch { toast.error('Error'); }
//         },
//       });
//     } else {
//       setReasonModal({
//         open: true,
//         title: 'Reject Offer — Enter Reason',
//         onConfirm: async (reason) => {
//           closeReason();
//           try {
//             await api.patch(`/hiring/documents/${id}/offer-status`, { status: 'Rejected', rejectionReason: reason });
//             toast.success('Offer rejected!'); fetchCand();
//           } catch { toast.error('Error'); }
//         },
//       });
//     }
...
//   /* ── Verify/Reject doc — Verified uses ConfirmModal, Rejected uses ReasonModal ── */
//   const verifyDoc = (docId, status) => {
//     if (status === 'Verified') {
//       setConfirm({
//         open: true,
//         title: 'Verify Document?',
//         message: <span>Mark this document as <strong>Verified</strong>? This action will be recorded.</span>,
//         danger: false,
//         onConfirm: async () => {
//           closeConfirm();
//           try {
//             await api.patch(`/hiring/documents/${id}/documents/${docId}/verify`, { status: 'Verified', remarks: '' });
//             toast.success('Document verified!'); fetchCand();
//           } catch { toast.error('Error'); }
//         },
//       });
//     } else {
...
//           closeReason();
//           try {
//             await api.patch(`/hiring/documents/${id}/documents/${docId}/verify`, { status: 'Rejected', remarks: reason });
//             toast.success('Document rejected!'); fetchCand();
//           } catch { toast.error('Error'); }
//         },
//       });
//     }
...
//             {cand.currentDesignation || ''} {cand.currentCompany ? `@ ${cand.currentCompany}` : ''}
//           </p>
//         </div>
//         <div style={{ display: 'flex', gap: '8px' }}>
//           <button className="btn btn-outline btn-sm" onClick={() => { setStageForm({ stage: cand.stage, note: '' }); setShowStageModal(true); }}>Update Stage</button>
...
//               <p style={{ color: 'var(--text-muted)', fontSize: '13px' }}>{cand.phone}</p>
//               {cand.resume?.url && (
//                 <a href={cand.resume.url} target="_blank" rel="noreferrer" className="btn btn-ghost btn-sm" style={{ marginTop: '14px', display: 'inline-flex' }}>
//                   📄 View Resume
//                 </a>
//               )}
//             </div>
...
//                   padding: '8px 0', borderBottom: '1px solid var(--border)', gap: '16px',
//                 }}>
//                   <span style={{ fontSize: '13px', color: 'var(--text-muted)', flexShrink: 0, minWidth: '80px' }}>{label}</span>
//                   <span style={{ fontSize: '13px', fontWeight: 600, textAlign: 'right', wordBreak: 'break-word' }}>{value || '—'}</span>
//                 </div>
//               ))}
//             </div>
...
//                   padding: '8px 0', borderBottom: '1px solid var(--border)', gap: '16px',
//                 }}>
//                   <span style={{ fontSize: '13px', color: 'var(--text-muted)', flexShrink: 0, minWidth: '110px' }}>{label}</span>
//                   <span style={{ fontSize: '13px', fontWeight: 600, textAlign: 'right', wordBreak: 'break-word' }}>{value || '—'}</span>
//                 </div>
//               ))}
//             </div>
...
//                 <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
//                   <div>
//                     <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Acceptance Status: </span>
//                     <span className={`badge badge-${cand.offer.acceptanceStatus === 'Accepted' ? 'green' : cand.offer.acceptanceStatus === 'Rejected' ? 'red' : 'amber'}`}>{cand.offer.acceptanceStatus}</span>
//                   </div>
//                   {cand.offer.offerLetterUrl && <a href={cand.offer.offerLetterUrl} target="_blank" rel="noreferrer" className="btn btn-ghost btn-sm">📄 View Offer Letter</a>}
//                   {cand.offer.acceptanceStatus === 'Pending' && (
//                     <>
...
//                 Status: <span className={`badge badge-${cand.onboardingStatus === 'Completed' ? 'green' : cand.onboardingStatus === 'Documents Pending' ? 'amber' : 'gray'}`}>{cand.onboardingStatus}</span>
//               </p>
//             </div>
//             <button className="btn btn-primary btn-sm" onClick={() => { setDocType(''); setDocFile(null); setDocName(''); setShowDocModal(true); }}>Upload Document</button>
//           </div>
...
//     <div className="modal">
//       <div className="modal-hdr">
//         <span className="modal-title">{editIntIdx !== null ? 'Edit Interview' : 'Schedule Interview'}</span>
//         <button className="close-btn" onClick={() => setShowIntModal(false)}>
//           <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
//             <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
//           </svg>
//         </button>
...
//             {/* ── PRE-INTERVIEW: Always visible ── */}
//             <div className="fg">
//               <label>Round Name</label>
//               <input
//                 value={intForm.round}
//                 onChange={e => setIntForm({ ...intForm, round: e.target.value })}
...
//             <div className="fg">
//               <label>Mode</label>
//               <select value={intForm.mode} onChange={e => setIntForm({ ...intForm, mode: e.target.value })}>
//                 {['In-person', 'Video Call', 'Phone', 'Technical Test', 'HR Round'].map(m => (
...
//                 onChange={e => setIntForm({ ...intForm, scheduledAt: e.target.value })}
//               />
//             </div>
...
//               <input
//                 value={intForm.meetingLink}
//                 onChange={e => setIntForm({ ...intForm, meetingLink: e.target.value })}
//                 placeholder="https://meet.google.com/..."
//               />
//             </div>

//             {/* ── POST-INTERVIEW: Sirf tab dikhao jab status = Completed ── */}
//             {intForm.status === 'Completed' && (
//               <>
//                 {/* Divider */}
...
//                     onChange={e => setIntForm({ ...intForm, rating: e.target.value })}
//                     placeholder="1 – 5"
//                   />
//                 </div>
...
//                     min="0"
//                     max="10"
//                     value={intForm.technicalScore}
//                     onChange={e => setIntForm({ ...intForm, technicalScore: e.target.value })}
//                     placeholder="0 – 10"
//                   />
//                 </div>
...
//                     min="0"
//                     max="10"
//                     value={intForm.communicationScore}
//                     onChange={e => setIntForm({ ...intForm, communicationScore: e.target.value })}
//                     placeholder="0 – 10"
//                   />
//                 </div>

//                 <div className="fg">
//                   <label>Culture Fit (0–10)</label>
//                   <input
//                     type="number"
//                     min="0"
//                     max="10"
//                     value={intForm.cultureFitScore}
//                     onChange={e => setIntForm({ ...intForm, cultureFitScore: e.target.value })}
//                     placeholder="0 – 10"
//                   />
//                 </div>
...
//                   <select
//                     value={intForm.recommendation}
//                     onChange={e => setIntForm({ ...intForm, recommendation: e.target.value })}
//                   >
//                     <option value="">Select</option>
//                     {['Strongly Recommend', 'Recommend', 'Neutral', 'Not Recommend', 'Strong Reject'].map(r => (
...
//                   <textarea
//                     value={intForm.feedback}
//                     onChange={e => setIntForm({ ...intForm, feedback: e.target.value })}
//                     rows={3}
...
//                   <textarea
//                     value={intForm.strengths}
//                     onChange={e => setIntForm({ ...intForm, strengths: e.target.value })}
//                     rows={2}
...
//                   <textarea
//                     value={intForm.weaknesses}
//                     onChange={e => setIntForm({ ...intForm, weaknesses: e.target.value })}
//                     rows={2}
...
//             {editIntIdx !== null ? 'Update' : 'Schedule Interview'}
//           </button>
//         </div>
//       </form>
//     </div>
...
export default function CandidateDetail() {
  ...
  const [intForm, setIntForm] = useState({
    ...
    meetingLink: '',
    status: 'Scheduled',
    // post-interview fields (only shown when status = Completed)
    rating: '',
    technicalScore: '',
    communicationScore: '',
    cultureFitScore: '',
  ...
  /* ── Helpers ── */
  const resetIntForm = () => setIntForm({
    round: '', mode: 'Video Call', scheduledAt: '', meetingLink: '', status: 'Scheduled',
    rating: '', technicalScore: '', communicationScore: '', cultureFitScore: '',
    feedback: '', strengths: '', weaknesses: '', recommendation: '',
  });
...
//   e.preventDefault();
//   if (!docFile) return toast.error('Select a file');
//   if (!docType) return toast.error('Select document type'); // ✅ extra validation
...
//   try {
//     await api.post(`/hiring/documents/${id}/documents`, fd, {
//       headers: { 'Content-Type': 'multipart/form-data' }, // ✅ sirf yeh chahiye
//       timeout: 60000,
//     });
//     toast.success('Document uploaded!');
//     setShowDocModal(false);
//     setDocFile(null); setDocType(''); setDocName('');
//     fetchCand();
//   } catch (err) {
...
const uploadDoc = async (e) => {
  e.preventDefault();
  if (!docFile) return toast.error('Select a file');
  if (!docType) return toast.error('Select document type');

  const fd = new FormData();
  fd.append('documents', docFile);  // ← field name 'documents' hona chahiye
  fd.append('type', docType);
  fd.append('name', docName || docFile.name);
  ...
  try {
    ...
    //   headers: { 'Content-Type': 'multipart/form-data' }, // ✅ Sirf yeh
    //   timeout: 60000,
    // });
    await api.post(`/hiring/documents/${id}/documents`, fd, {
  ...
  /* ── Shared int form open handler ── */
  const openAddIntModal = () => { resetIntForm(); setEditIntIdx(null); setShowIntModal(true); };
  const openEditIntModal = (iv, i) => {
    setIntForm({
      round: iv.round || '',
      mode: iv.mode || 'Video Call',
      scheduledAt: iv.scheduledAt ? iv.scheduledAt.split('T')[0] + 'T' + iv.scheduledAt.split('T')[1]?.slice(0, 5) : '',
      meetingLink: iv.meetingLink || '',
      status: iv.status || 'Scheduled',
      rating: iv.rating || '',
      technicalScore: iv.technicalScore || '',
      communicationScore: iv.communicationScore || '',
      cultureFitScore: iv.cultureFitScore || '',
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="btn btn-outline btn-sm" onClick={() => { setStageForm({ stage: cand.stage, note: '' }); setShowStageModal(true); }}>Update Stage</button>
          <button className="btn btn-outline btn-sm" onClick={openAddIntModal}>+ Interview</button>
          {!offerSent && cand.stage === 'Selected' && (
            <button className="btn btn-success btn-sm" onClick={() => setShowOfferModal(true)}>Send Offer</button>
          )}
        </div>
      </div>
      ...
      {tab === 'interviews' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '16px' }}>
            <button className="btn btn-primary" onClick={openAddIntModal}>+ Schedule Interview</button>
          </div>
          {cand.interviews?.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {cand.interviews.map((iv, i) => (
                <div key={iv._id} className="card">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                      {iv.rating && <span style={{ fontSize: '14px' }}>{'⭐'.repeat(iv.rating)}</span>}
                      <button className="btn btn-ghost btn-sm" onClick={() => openEditIntModal(iv, i)}>Edit</button>
                    </div>
                  </div>
                  {(iv.technicalScore || iv.communicationScore || iv.cultureFitScore) && (
                    <div style={{ display: 'flex', gap: '16px', marginBottom: '10px', padding: '10px', background: 'var(--bg-2)', borderRadius: '8px' }}>
                  ...
                  {iv.feedback && (
                    <div style={{ marginBottom: '8px' }}>
                      <span style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px', fontWeight: 700 }}>Feedback</span>
                      <p style={{ fontSize: '13px', color: 'var(--text-dim)', marginTop: '3px', lineHeight: 1.6 }}>{iv.feedback}</p>
                    </div>
                  )}
                  {iv.strengths && <div style={{ marginBottom: '4px' }}><span style={{ fontSize: '11px', color: 'var(--success)', fontWeight: 700 }}>STRENGTHS: </span><span style={{ fontSize: '13px' }}>{iv.strengths}</span></div>}
                  {iv.weaknesses && <div style={{ marginBottom: '4px' }}><span style={{ fontSize: '11px', color: 'var(--danger)', fontWeight: 700 }}>AREAS OF IMPROVEMENT: </span><span style={{ fontSize: '13px' }}>{iv.weaknesses}</span></div>}
                  {iv.recommendation && (
                    <div style={{ marginTop: '8px' }}>
                      <span className={`badge badge-${iv.recommendation.includes('Recommend') && !iv.recommendation.includes('Not') ? 'green' : iv.recommendation.includes('Neutral') ? 'amber' : 'red'}`}>{iv.recommendation}</span>
                    </div>
                  )}
                  {iv.meetingLink && (
                    <a href={iv.meetingLink} target="_blank" rel="noreferrer" className="btn btn-ghost btn-sm" style={{ marginTop: '10px', display: 'inline-flex' }}>🔗 Join Meeting</a>
                  )}
                </div>
              ))}
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="card">
            <div className="card-hdr">
              <span className="card-title">Offer Details</span>
              {!offerSent && cand.stage === 'Selected' && (
                <button className="btn btn-success btn-sm" onClick={() => setShowOfferModal(true)}>Generate Offer</button>
              )}
            </div>
            {cand.offer?.salary || cand.offer?.stipend ? (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                  {[
                    ['Salary / Stipend', cand.offer.salary ? fmtMoney(cand.offer.salary) : fmtMoney(cand.offer.stipend)],
                    ['Designation', cand.offer.designation || '—'],
                    ['Joining Date', fmtDate(cand.offer.joiningDate)],
                  ].map(([l, v]) => (
                    <div key={l} style={{ textAlign: 'center', padding: '16px', background: 'var(--bg-2)', borderRadius: '10px' }}>
                      <div style={{ fontSize: '20px', fontWeight: 800, fontFamily: 'var(--font-display)', color: 'var(--primary)' }}>{v}</div>
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>{l}</div>
                ...
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  {cand.offer.offerLetterUrl && (
                    <a href={cand.offer.offerLetterUrl} target="_blank" rel="noreferrer" className="btn btn-ghost btn-sm">📄 View Offer Letter</a>
                  )}
                  {cand.offer.acceptanceStatus === 'Pending' && (
                    <>
                      <button className="btn btn-success btn-sm" onClick={() => updateOfferStatus('Accepted')}>Mark Accepted</button>
                  ...
                  {cand.offer.offerLetterSentAt && (
                    <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Sent: {fmtDate(cand.offer.offerLetterSentAt)}</span>
                  )}
                </div>
              </>
            ) : (
              <div className="empty">
                ...
                <span>Mark candidate as 'Selected' first, then generate offer</span>
              </div>
            )}
          </div>
          {cand.payslips?.length > 0 && (
      ...
      {showStageModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowStageModal(false)}>
          <div className="modal modal-sm">
            <div className="modal-hdr">
              <span className="modal-title">Update Stage</span>
              <button className="close-btn" onClick={() => setShowStageModal(false)}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
                  <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
                </svg>
              </button>
            ...
            <form onSubmit={updateStage}>
              <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div className="fg">
                  <label>Note</label>
                  <textarea value={stageForm.note} onChange={e => setStageForm({ ...stageForm, note: e.target.value })} placeholder="Add a note about this stage change..." />
                </div>
              </div>
              <div className="modal-foot">
                <button type="button" className="btn btn-ghost" onClick={() => setShowStageModal(false)}>Cancel</button>
      ...
      {/* ════════════════════════════════════
          ...
          ── PRE-INTERVIEW fields: always visible
          ── POST-INTERVIEW fields: only when status = Completed
      ════════════════════════════════════ */}
      {showIntModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowIntModal(false)}>
          <div className="modal">
            ...
            <form onSubmit={addInterview}>
              <div className="modal-body">
                <div className="form-grid">
                  ...
                  {intForm.status === 'Completed' && (
                    <>
                      {/* Section divider */}
                      <div className="fg full" style={{ borderTop: '1px dashed var(--border)', paddingTop: '16px', marginTop: '4px' }}>
                        <p style={{
                        ...
                        <p style={{ fontSize: '12px', color: 'var(--text-muted)', margin: 0 }}>
                          Interview ho gaya — ab scores aur feedback fill karein
                        </p>
                      </div>
              ...
              <div className="modal-foot">
                <button type="button" className="btn btn-ghost" onClick={() => setShowIntModal(false)}>Cancel</button>
                <button type="submit" className="btn btn-primary">
                  {editIntIdx !== null ? 'Update Interview' : 'Schedule Interview'}
                </button>
              </div>
            </form>
          </div>
      ...
      {/* ════════════════════════════════════
          MODAL: Upload Offer Letter
      ════════════════════════════════════ */}
      {showOfferModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowOfferModal(false)}>
          <div className="modal modal-sm">
            <div className="modal-hdr">
              <span className="modal-title">Upload Offer Letter</span>
            ...
            <form onSubmit={uploadOffer}>
              <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div className="fg">
                  ...
                  <input type="number" value={offerForm.salary} onChange={e => setOfferForm({ ...offerForm, salary: e.target.value })} placeholder="1500000" />
                </div>
                <div className="fg">
                  <label>Stipend (₹) — for interns</label>
                  <input type="number" value={offerForm.stipend} onChange={e => setOfferForm({ ...offerForm, stipend: e.target.value })} />
                </div>
                <div className="fg">
                  <label>Designation</label>
                  <input value={offerForm.designation} onChange={e => setOfferForm({ ...offerForm, designation: e.target.value })} placeholder="Senior Developer" />
                </div>
                <div className="fg">
                  <label>Joining Date</label>
                  <input type="date" value={offerForm.joiningDate} onChange={e => setOfferForm({ ...offerForm, joiningDate: e.target.value })} />
                </div>
                <div className="fg">
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowDocModal(false)}>
          <div className="modal modal-sm">
            <div className="modal-hdr">
              ...
              <button className="close-btn" onClick={() => setShowDocModal(false)}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
                  <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
                </svg>
              </button>
            ...
            <form onSubmit={uploadDoc}>
              <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div className="fg">
                  ...
                  <input value={docName} onChange={e => setDocName(e.target.value)} placeholder="Auto-filled from file name" />
                </div>
                <div className="fg">
                  <label>File *</label>
                  <div className="upload-zone" onClick={() => document.getElementById('docUpload').click()}>
                    ...
                    <p>{docFile ? docFile.name : 'Click to upload'}</p>
                    <span className="sub">PDF, DOC, JPG, PNG</span>
                  </div>
                  <input id="docUpload" type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" style={{ display: 'none' }} onChange={e => setDocFile(e.target.files[0])} />
                </div>
              </div>
