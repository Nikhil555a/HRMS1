...
export default function Jobs() {
  ...
  const openAdd = () => {
    setEditJob(null); setForm(initForm);
    setSkillInput(''); setReqInput(''); setShowModal(true);
  };
  ...
  const handleDelete = (job) => {
    openConfirm(
      'Delete Job?',
      <span>
        You are about to permanently delete <strong>{job.title}</strong>.
        All associated data will be removed and this cannot be undone.
      </span>,
      async () => {
        closeConfirm();
        try {
          await api.delete(`/hiring/jobs/${job._id}`);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {loading ? (
          ...
          <div
            key={job._id}
            className="card"
            style={{ borderLeft: `3px solid var(--${priorityColor[job.priority] === 'gray' ? 'border' : (job.priority === 'Urgent' ? 'danger' : job.priority === 'High' ? 'accent' : 'primary')})` }}
          >
            <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap', marginBottom: '6px' }}>
                  ...
                  <div style={{ marginTop: '10px', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {job.platforms.map(p => (
                      <span key={p._id} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: '6px', padding: '3px 8px', fontSize: '12px' }}>
                        ...
                        {p.url && <a href={p.url} target="_blank" rel="noreferrer" style={{ color: 'var(--primary)', fontSize: '10px' }}>↗</a>}
                        <button
                          onClick={() => handleRemovePlatform(job._id, p._id)}
                    ...
                    <button
                      ...
                      onClick={() => { setSelectedJob(job); setPlatForm({ name: '', url: '' }); setShowPlatformModal(true); }}
                    >+ Platform</button>
                  </div>
                )}
                {job.platforms?.length === 0 && (
              ...
              <div style={{ display: 'flex', gap: '6px', flexShrink: 0, flexWrap: 'wrap' }}>
                ...
                <button className="btn btn-ghost btn-sm" onClick={() => openEdit(job)}>Edit</button>
                <button className="btn btn-danger btn-sm" onClick={() => handleDelete(job)}>Delete</button>
              </div>
            </div>
          </div>
        ))}
              ...
              <div className="modal-body">
                ...
                <div className="form-grid">
                  ...
                  <div className="fg">
                    ...
                    <input value={form.location} onChange={e => set('location', e.target.value)} required placeholder="Bangalore / Remote" />
                  </div>
                  <div className="fg">
                  ...
                  <div className="fg">
                    ...
                    <input value={form.experienceRequired} onChange={e => set('experienceRequired', e.target.value)} placeholder="2-5 years" />
                  </div>
                  <div className="fg">
                  ...
                  <div className="fg">
                    ...
                    <input type="number" value={form.salaryMin} onChange={e => set('salaryMin', e.target.value)} placeholder="1200000" />
                  </div>
                  <div className="fg">
                    <label>Max Salary (₹)</label>
                    <input type="number" value={form.salaryMax} onChange={e => set('salaryMax', e.target.value)} placeholder="2000000" />
                  </div>
                  <div className="fg">
                  ...
                  {currentUser?.role === 'super_admin' && (
                    <div className="fg">
                      ...
                      <select value={form.postedBy} onChange={e => set('postedBy', e.target.value)} required={!editJob}>
                        <option value="">— Select HR —</option>
                        {hrs.map(hr => <option key={hr._id} value={hr._id}>{hr.name} ({hr.email})</option>)}
                      </select>
                    </div>
                  )}
                  <div className="fg full">
                    <label>Job Description *</label>
                    <textarea value={form.description} onChange={e => set('description', e.target.value)} required rows={4} placeholder="Describe the role, responsibilities..." />
                  </div>
                </div>
                ...
                <div className="form-grid">
                  <div className="fg full">
                    ...
                    <div className="skills-wrap" style={{ marginTop: '8px' }}>
                      {form.skills.map(s => (
                        <span key={s} className="skill-pill" style={{ display: 'inline-flex', gap: '4px', cursor: 'default' }}>
                          {s}
                          <button type="button" onClick={() => removeTag('skills', s)} style={{ background: 'none', border: 'none', color: 'inherit', cursor: 'pointer' }}>×</button>
                        </span>
                      ))}
                    </div>
                  </div>
                  <div className="fg full">
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '8px' }}>
                      {form.requirements.map(r => (
                        <span key={r} style={{ background: 'rgba(245,158,11,0.1)', color: '#fbbf24', borderRadius: '5px', padding: '3px 9px', fontSize: '12px', display: 'inline-flex', gap: '4px' }}>
                          {r}
                          <button type="button" onClick={() => removeTag('requirements', r)} style={{ background: 'none', border: 'none', color: 'inherit', cursor: 'pointer' }}>×</button>
                        </span>
                      ))}
                    </div>
                  </div>
      ...
      {showPlatformModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowPlatformModal(false)}>
          <div className="modal modal-sm">
            <div className="modal-hdr">
              <span className="modal-title">Add Job Platform</span>
              <button className="close-btn" onClick={() => setShowPlatformModal(false)}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
                  <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
                </svg>
              </button>
            ...
            <form onSubmit={handleAddPlatform}>
              <div className="modal-body">
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  ...
                  <div className="fg">
                    <label>Job URL</label>
                    <input value={platForm.url} onChange={e => setPlatForm({ ...platForm, url: e.target.value })} placeholder="https://linkedin.com/jobs/123" />
                  </div>
                </div>
              </div>
              <div className="modal-foot">
