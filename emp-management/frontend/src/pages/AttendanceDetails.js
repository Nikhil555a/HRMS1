...
// ── 2026 Holidays (DEMO_HOLIDAYS_2026 se match) ────────────
const HOLIDAYS_2026 = {
  '2026-01-26': { name: 'Republic Day',             type: 'national', icon: '🇮🇳', description: 'National celebration of the Constitution of India' },
  '2026-03-04': { name: 'Holi',                     type: 'festival', icon: '🎨', description: 'Festival of Colors' },
  '2026-03-21': { name: 'Eid-ul-Fitr (Ramzan Eid)', type: 'festival', icon: '🕌', description: 'End of Ramadan (tentative)' },
  '2026-04-03': { name: 'Good Friday',              type: 'national', icon: '✝️', description: 'Christian observance' },
  '2026-05-27': { name: 'Eid-ul-Adha (Bakrid)',     type: 'festival', icon: '🕌', description: 'Islamic festival of sacrifice (tentative)' },
  '2026-08-15': { name: 'Independence Day',         type: 'national', icon: '🇮🇳', description: "India's Independence Day" },
  '2026-09-04': { name: 'Janmashtami',              type: 'festival', icon: '🪗', description: 'Birth of Lord Krishna' },
  '2026-10-02': { name: 'Gandhi Jayanti',           type: 'national', icon: '🕊️', description: 'Birthday of Mahatma Gandhi' },
  '2026-10-18': { name: 'Durga Puja – Maha Saptami',type: 'festival', icon: '🪔', description: 'Common Festive – Durga Puja day' },
  '2026-10-19': { name: 'Durga Puja – Maha Ashtami',type: 'festival', icon: '🪔', description: 'Common Festive – Durga Puja day' },
  '2026-10-20': { name: 'Durga Puja – Maha Navami', type: 'festival', icon: '🪔', description: 'Common Festive – Durga Puja day' },
  '2026-10-21': { name: 'Dussehra (Vijayadashami)', type: 'national', icon: '🏹', description: 'Victory of Good over Evil' },
  '2026-11-08': { name: 'Diwali (Deepavali)',        type: 'festival', icon: '🪔', description: 'Festival of Lights' },
  '2026-12-25': { name: 'Christmas Day',             type: 'national', icon: '🎄', description: 'Christmas Day' },
};
...
const STATUS_CONFIG = {
  Present:          { color: '#10b981', bg: 'rgba(16,185,129,0.18)',  label: 'P',   short: 'Present'        },
  Absent:           { color: '#ef4444', bg: 'rgba(239,68,68,0.18)',   label: 'A',   short: 'Absent'         },
  'Half Day':       { color: '#f59e0b', bg: 'rgba(245,158,11,0.18)',  label: 'H',   short: 'Half Day'       },
  Late:             { color: '#f59e0b', bg: 'rgba(245,158,11,0.18)',  label: 'L',   short: 'Late'           },
  'On Leave':       { color: '#818cf8', bg: 'rgba(99,102,241,0.18)',  label: 'OL',  short: 'Leave'          },
  Holiday:          { color: '#22d3ee', bg: 'rgba(34,211,238,0.18)',  label: 'HO',  short: 'Holiday'        },
  'Work From Home': { color: '#22d3ee', bg: 'rgba(34,211,238,0.18)',  label: 'WFH', short: 'Work From Home' },
};

const HOLIDAY_TYPE_STYLE = {
  national: { color: '#93c5fd', bg: 'rgba(29,78,216,0.18)',  border: 'rgba(147,197,253,0.3)', label: 'National' },
  festival: { color: '#c4b5fd', bg: 'rgba(124,58,237,0.18)', border: 'rgba(196,181,253,0.3)', label: 'Festival' },
  company:  { color: '#7dd3fc', bg: 'rgba(3,105,161,0.18)',  border: 'rgba(125,211,252,0.3)', label: 'Company'  },
};
...
function isSaturday(d) { return d.getDay() === 6; }
function getDaysInMonth(y, m) { return new Date(y, m, 0).getDate(); }
function isWeekend(d, satOff) { return d.getDay() === 0 || (d.getDay() === 6 && satOff); }
...
async function fetchHolidaysForYear(year) {
  const key = String(year);
  if (holidayCache[key]) return holidayCache[key];
  const map = { ...COMPANY_HOLIDAYS, ...getHolidaysForYear(year) };
  holidayCache[key] = map;
  return map;
}
...
function StatCard({ label, value, color, sub, highlight }) {
  return (
    <div className="card" style={{
      padding: '14px 12px',
      borderLeft: highlight ? `3px solid ${color}` : '3px solid var(--border)',
      background: highlight ? `${color}22` : undefined,
    }}>
      <div style={{ fontSize: '22px', fontWeight: 800, color, lineHeight: 1, letterSpacing: '-0.5px' }}>
        {value}
      </div>
      ...
      {sub && (
        <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '2px', opacity: 0.7 }}>
          {sub}
        </div>
      )}
    </div>
...
export default function AttendanceDetail({ employee, onClose }) {
  ...
  const [selectedDay, setSelectedDay]         = useState(null);
  const [allHolidays, setAllHolidays]         = useState(() => ({
    ...COMPANY_HOLIDAYS, ...getHolidaysForYear(now.getFullYear()),
  }));
  const [holidaysLoading, setHolidaysLoading] = useState(false);
  const [satOff, setSatOff]                   = useState(false);
  const [activeTab, setActiveTab]             = useState('calendar');
  ...
  function getCellBg(cell) {
    ...
    if (cell.sun)    return { bg: 'rgba(239,68,68,0.12)',  color: '#f87171' };
    if (cell.satHol) return { bg: 'rgba(244,63,94,0.12)',  color: '#fb7185' };
    if (cell.satWork) {
      if (cell.record) {
        const cfg = STATUS_CONFIG[cell.record.status] || {};
        return { bg: cfg.bg, color: cfg.color, satBorder: true };
      }
      if (cell.future) return { bg: 'var(--bg)', color: 'var(--text-muted)', satBorder: true };
      return { bg: 'rgba(239,68,68,0.10)', color: '#f87171', satBorder: true };
    }
    if (cell.record) {
      const cfg = STATUS_CONFIG[cell.record.status] || {};
      return { bg: cfg.bg || 'var(--bg)', color: cfg.color || 'var(--text)' };
    }
    if (cell.future) return { bg: 'var(--bg)', color: 'var(--text-muted)' };
    return { bg: 'rgba(239,68,68,0.10)', color: '#f87171' };
  }

  function getCellBadge(cell) {
    if (cell.holiday)  return null;
    if (cell.sun)      return { text: 'OFF', color: '#f87171' };
    if (cell.satHol)   return { text: 'OFF', color: '#fb7185' };
    if (cell.satWork) {
      if (cell.record)  return { text: STATUS_CONFIG[cell.record.status]?.label || '?', color: STATUS_CONFIG[cell.record.status]?.color };
      if (!cell.future) return { text: '?', color: '#f87171' };
      return null;
    }
    if (cell.record)   return { text: STATUS_CONFIG[cell.record.status]?.label || '?', color: STATUS_CONFIG[cell.record.status]?.color };
    if (!cell.future)  return { text: '?', color: '#f87171' };
  ...
  function getDayHeaderStyle(day) {
    if (day === 'Sun') return { color: '#f87171', fontWeight: 700 };
    if (day === 'Sat') return satOff
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{
        ...
        <div className="modal-header" style={{ padding: '16px 20px', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginLeft: 'auto' }}>
            ...
            <div style={{
              ...
              {MONTHS[month - 1]} {year}
              {holidaysLoading && (
                <span style={{ fontSize: '9px', color: 'var(--text-muted)', display: 'block', fontWeight: 400 }}>
                  Syncing…
                </span>
              )}
            </div>
        ...
        <div style={{
          ...
          {[
            { val: false, icon: '💼', label: 'Working', activeBg: 'rgba(99,102,241,0.12)', ac: 'var(--primary)', ab: 'var(--primary)' },
            { val: true,  icon: '🏖️', label: 'Holiday', activeBg: 'rgba(239,68,68,0.12)',  ac: 'var(--danger)',  ab: 'var(--danger)'  },
          ].map(opt => (
            <button
              key={String(opt.val)}
              onClick={() => setSatOff(opt.val)}
              style={{
                ...
                padding: '5px 14px', borderRadius: '20px', fontSize: '12px',
                fontWeight: 600, cursor: 'pointer', transition: 'all 0.18s',
                border: `1.5px solid ${satOff === opt.val ? opt.ab : 'var(--border)'}`,
                background: satOff === opt.val ? opt.activeBg : 'transparent',
                color: satOff === opt.val ? opt.ac : 'var(--text-muted)',
              }}
            >
              {opt.icon} Saturday {opt.label}
            </button>
          ...
          {loading ? (
            ...
            <>
              ...
              <div style={{
                ...
                <StatCard label="Working Days" value={workingDays}         color="var(--text-muted)" sub={satOff ? 'Mon–Fri' : 'Mon–Sat'} />
                <StatCard label="Present"      value={stats.present}       color="#10b981" />
                <StatCard label="Absent"       value={stats.absent}        color="#ef4444" />
                <StatCard label="On Leave"     value={stats.leave}         color="#818cf8" />
                <StatCard label="Half Day"     value={stats.halfDay}       color="#f59e0b" />
                <StatCard label="Late"         value={stats.late}          color="#f59e0b" />
                <StatCard label="WFH"          value={stats.wfh}           color="#22d3ee" />
                <StatCard label="Attendance"   value={`${attendancePct}%`} color={pctColor} sub="of working days" highlight />
              ...
              <div className="card" style={{ padding: '14px 16px', marginBottom: '20px' }}>
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontWeight: 800, color: pctColor, fontSize: '15px' }}>{attendancePct}%</span>
                  </div>
                </div>
                <div style={{
                  height: '10px', borderRadius: '5px', overflow: 'hidden',
                  ...
                  <div style={{
                    position: 'absolute', left: '75%', top: 0, bottom: 0,
                    width: '1.5px', background: 'var(--border)', zIndex: 1,
                  }} />
                  <div style={{
                    height: '100%', width: `${attendancePct}%`,
                    background: `linear-gradient(90deg, ${pctColor}99, ${pctColor})`,
                ...
                <div style={{
                  ...
                  <span>0%</span><span>▲ 75% min</span><span>100%</span>
                </div>
              </div>
                ...
                <>
                  <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: '16px' }}>
                      ...
                      {DAYS.map(day => (
                        <div key={day} style={{
                          ...
                          {day === 'Sat' && (
                            <div style={{
                              fontSize: '7px', fontWeight: 700,
                              background: satOff ? 'rgba(244,63,94,0.18)' : 'rgba(99,102,241,0.18)',
                              color: satOff ? '#fb7185' : '#93c5fd',
                              borderRadius: '3px', padding: '1px 3px',
                              marginTop: '2px', display: 'inline-block',
                            }}>
                              {satOff ? 'OFF' : 'WORK'}
                    ...
                    <div style={{
                      ...
                      {calendarCells.map(cell => {
                        if (cell.empty) return (
                          <div key={cell.key} style={{ background: 'var(--bg)', minHeight: '72px' }} />
                        );
                        const { bg, color, satBorder } = getCellBg(cell);
                        const badge = getCellBadge(cell);
                        return (
                          <div
                            ...
                            style={{
                              ...
                              cursor: 'pointer', position: 'relative',
                              transition: 'filter 0.15s',
                              borderLeft: satBorder ? '3px solid var(--primary)' : undefined,
                            }}
                          ...
                          >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <span style={{
                                ...
                                borderRadius: '50%', width: 20, height: 20,
                                display: 'flex', alignItems: 'center', justifyContent: 'center',
                                boxShadow: cell.isToday ? '0 0 0 2px rgba(99,102,241,0.4)' : 'none',
                              }}>
                                {cell.d}
                              </span>
                              {badge && (
                                <span style={{ fontSize: '9px', fontWeight: 700, color: badge.color }}>
                                  {badge.text}
                                </span>
                              )}
                            </div>
                            ...
                            {cell.holiday && (
                              <div style={{ marginTop: '3px' }}>
                                <HolidayChip holiday={cell.holiday} />
                                <div style={{
                                  fontSize: '8px', marginTop: '2px', color,
                            ...
                            {!cell.holiday && cell.sun && (
                              <div style={{ fontSize: '8px', marginTop: '3px', color: '#f87171', fontWeight: 600 }}>Sunday Off</div>
                            )}
                            {!cell.holiday && cell.satHol && (
                              <div style={{ fontSize: '8px', marginTop: '3px', color: '#fb7185', fontWeight: 600 }}>Sat Holiday</div>
                            )}
                            {!cell.holiday && cell.satWork && (
                              <div style={{ fontSize: '8px', marginTop: '3px', color: '#93c5fd', fontWeight: 600 }}>Sat Working</div>
                            )}

                            {cell.record?.checkIn && (
                              <div style={{ fontSize: '9px', marginTop: '2px', color, fontFamily: 'Space Mono, monospace', fontWeight: 600 }}>
                                ↑ {new Date(cell.record.checkIn).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                              </div>
                            )}
                            {cell.record?.workHours > 0 && (
                              <div style={{ fontSize: '8px', marginTop: '1px', color: 'var(--text-muted)', fontWeight: 500 }}>
                                {cell.record.workHours.toFixed(1)}h
                              </div>
                            )}
                          </div>
                  ...
                  <div className="card" style={{ padding: '12px 14px', marginBottom: '16px' }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 14px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                        <span style={{ width: 10, height: 10, borderRadius: '2px', display: 'inline-block', background: 'rgba(239,68,68,0.18)', border: '1.5px solid #f87171' }} />
                        <span style={{ fontSize: '10px', color: '#f87171', fontWeight: 700 }}>Sunday Off</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                        <span style={{ width: 10, height: 10, borderRadius: '2px', display: 'inline-block', background: 'rgba(29,78,216,0.18)', border: '1.5px solid rgba(147,197,253,0.5)' }} />
                        <span style={{ fontSize: '10px', color: '#93c5fd', fontWeight: 700 }}>🇮🇳 National Holiday</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                        <span style={{ width: 10, height: 10, borderRadius: '2px', display: 'inline-block', background: 'rgba(124,58,237,0.18)', border: '1.5px solid rgba(196,181,253,0.5)' }} />
                        <span style={{ fontSize: '10px', color: '#c4b5fd', fontWeight: 700 }}>🎉 Festival Holiday</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                        <span style={{ width: 10, height: 10, borderRadius: '2px', display: 'inline-block', background: 'rgba(239,68,68,0.10)', border: '1.5px solid rgba(239,68,68,0.3)' }} />
                        <span style={{ fontSize: '10px', color: '#f87171' }}>? Not Marked</span>
                      </div>
                    </div>
                  </div>
                </>
                ...
                <div>
                  ...
                  {monthHolidays.length === 0 ? (
                    <div className="empty-state">
                      ...
                      <span>Enjoy your working days! 💪</span>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {monthHolidays.map(h => {
                        ...
                        return (
                          <div key={h.dk} style={{
                            ...
                            <div style={{ fontSize: '26px', lineHeight: 1, minWidth: '32px', textAlign: 'center' }}>
                              {h.icon}
                            </div>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontWeight: 700, fontSize: '13px', color: ts.color }}>{h.name}</div>
                  ...
                  <div className="card" style={{
                    ...
                    <span style={{ color: 'var(--text-muted)' }}>Total holidays in {year}:</span>
                    <span style={{ fontWeight: 700, color: 'var(--primary)' }}>
                      {Object.keys(allHolidays).length} days
                    </span>
                  </div>
                </div>
              )}
      ...
      {selectedDay && (
        <div className="modal-overlay" style={{ zIndex: 1100 }} onClick={() => setSelectedDay(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '340px', width: '100%' }}>
            <div className="modal-header" style={{ padding: '14px 16px' }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: '14px' }}>
                  {selectedDay.obj.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}
                ...
                {selectedDay.holiday && (
                  <div style={{ marginTop: '4px' }}><HolidayChip holiday={selectedDay.holiday} /></div>
                )}
              </div>
              <button className="modal-close" onClick={() => setSelectedDay(null)}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
            ...
            <div className="modal-body" style={{ padding: '16px' }}>
              {selectedDay.holiday ? (
                <div style={{ textAlign: 'center', padding: '16px 0' }}>
                  <div style={{ fontSize: '44px', lineHeight: 1, marginBottom: '12px' }}>{selectedDay.holiday.icon}</div>
                  <div style={{ fontWeight: 800, fontSize: '15px', color: (HOLIDAY_TYPE_STYLE[selectedDay.holiday.type] || HOLIDAY_TYPE_STYLE.festival).color, marginBottom: '8px' }}>
                    {selectedDay.holiday.name}
                  </div>
                ...
                <div style={{ textAlign: 'center', padding: '16px 0' }}>
                  <div style={{ fontSize: '44px', marginBottom: '10px' }}>🔴</div>
                  <div style={{ fontWeight: 700, color: '#f87171', fontSize: '14px' }}>Sunday — Weekly Off</div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '6px' }}>Not counted in working days</div>
                </div>
              ) : selectedDay.satHol ? (
                <div style={{ textAlign: 'center', padding: '16px 0' }}>
                  <div style={{ fontSize: '44px', marginBottom: '10px' }}>🏖️</div>
                  <div style={{ fontWeight: 700, color: '#fb7185', fontSize: '14px' }}>Saturday — Weekly Off</div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '6px' }}>Policy: Saturday Holiday is active</div>
                </div>
              ) : selectedDay.satWork && !selectedDay.record ? (
                <div style={{ textAlign: 'center', padding: '16px 0' }}>
                  <div style={{ fontSize: '44px', marginBottom: '10px' }}>💼</div>
                  <div style={{ fontWeight: 700, color: '#93c5fd', fontSize: '14px' }}>Saturday — Working Day</div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '6px' }}>No attendance record found</div>
                </div>
              ) : selectedDay.record ? (
                <div>
                  {(() => {
                  ...
                  {[
                    ['Check In',     selectedDay.record.checkIn  ? new Date(selectedDay.record.checkIn).toLocaleTimeString('en-IN',  { hour: '2-digit', minute: '2-digit' }) : null],
                    ['Check Out',    selectedDay.record.checkOut ? new Date(selectedDay.record.checkOut).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : null],
                    ['Work Hours',   selectedDay.record.workHours > 0 ? `${selectedDay.record.workHours.toFixed(1)}h` : null],
                    ['Leave Type',   selectedDay.record.leaveType   || null],
                    ['Leave Reason', selectedDay.record.leaveReason || null],
                    ['Notes',        selectedDay.record.notes       || null],
                ...
                <div className="empty-state">
                  <p>No record</p>
                  <span>No attendance found for this day</span>
                </div>
              )}
            </div>
          </div>
