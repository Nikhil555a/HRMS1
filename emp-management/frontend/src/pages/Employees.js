...
function DeleteModal({ emp, onConfirm, onCancel }) {
  return (
      ...
      <div className="modal" style={{ maxWidth: '400px' }}>
        ...
        <div className="modal-body">
          <p style={{ fontSize: '15px', lineHeight: 1.6 }}>
            Are you sure you want to delete <strong>{emp?.firstName} {emp?.lastName}</strong>? This action cannot be undone.
          </p>
        </div>
    ...
    <div>
      ...
      <div className="filters-bar">
        ...
        {(search || filterDept || filterStatus) && (
          <button className="btn btn-secondary btn-sm" onClick={() => { setSearch(''); setFilterDept(''); setFilterStatus(''); }}>
            Clear
          </button>
        )}
      </div>
                    ...
                    <tr key={emp._id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/employees/${emp._id}`)}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          {emp.profilePhoto
                            ...
                            : <div className="emp-avatar" style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 700 }}>
                      ...
                      <td>
                        <select
                          ...
                          style={{
                            ...
                            padding: '3px 6px', fontSize: '11px', fontWeight: 700,
                            textTransform: 'uppercase', letterSpacing: '0.5px',
                            color: statusColors[emp.status] === 'success' ? 'var(--success)'
                              : statusColors[emp.status] === 'warning' ? 'var(--warning)'
                              : statusColors[emp.status] === 'danger' ? 'var(--danger)'
                              : 'var(--text-muted)'
                          }}
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: '6px' }} onClick={e => e.stopPropagation()}>
                          ...
                          <button className="btn btn-danger btn-sm" onClick={e => { e.stopPropagation(); setDeleteTarget(emp); }}>Del</button>
                        </div>
                      </td>
                    </tr>
                  ))}
