...
// // Required fields per tab — for cross-tab validation
// const REQUIRED_FIELDS = {
//   basic:     ['employeeId', 'firstName', 'lastName', 'email', 'phone'],
//   skills:    ['designation'],
//   documents: ['joiningDate'],
// };
...
//     password:        '',  // always start empty — send only if filled
...
//   const validateAll = () => {
//     for (const [tabName, fields] of Object.entries(REQUIRED_FIELDS)) {
//       for (const field of fields) {
//         if (!form[field] || String(form[field]).trim() === '') {
//           const labels = {
//             employeeId: 'Employee ID', firstName: 'First Name', lastName: 'Last Name',
//             email: 'Email', phone: 'Phone', designation: 'Designation', joiningDate: 'Joining Date',
//           };
//           toast.error(`"${labels[field] || field}" is required (${tabName} tab)`);
//           setTab(tabName);
//           return false;
//         }
//       }
...
//       toast.error('Password must be at least 6 characters (Basic Info tab)');
//       setTab('basic');
//       return false;
//     }
//     // Password min-length on edit (if filled)
//     if (isEdit && form.password && form.password.length < 6) {
...
//         'alternatePhone', 'dateOfBirth', 'gender', 'department',
//         'designation', 'appliedPosition', 'employmentType', 'status',
//         'joiningDate', 'closingDate', 'salary', 'previousCompany', 'bloodGroup',
//       ];
//       const requiredScalar = ['employeeId','firstName','lastName','email','phone','designation','joiningDate','department','employmentType','status'];
//       scalarFields.forEach(key => {
//         const val = form[key];
//         if (requiredScalar.includes(key) || (val !== '' && val !== null && val !== undefined)) {
//           fd.append(key, val);
//         }
//       });
...
//       // Skills — comma-separated (empty string clears on backend)
...
//                     />
//                     <PwdToggleBtn show={showPwd} onToggle={() => setShowPwd(s => !s)} />
//                   </div>
...
//                       <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>No skills added yet</span>
//                     )}
//                   </div>
//                 </div>
//               </div>
...
export default function EmployeeForm({ employee, departments, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    previousCompany: employee?.previousCompany || '',
    bloodGroup:      employee?.bloodGroup      || '',
    // ✅ FIXED: Edit mein current password prefill hoga
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Password — hamesha bhejo agar filled hai
      if (form.password) fd.append('password', form.password);
          ...
          <div className="modal-body">
            ...
            {tab === 'basic' && (
              <div className="form-grid">
                ...
                {/* ✅ Password — edit mein current dikhega */}
                <div className="form-group">
                  <label>{isEdit ? 'Password (current dikhega, change karo ya chhodo)' : 'Password *'}</label>
                  <div style={{ position: 'relative' }}>
                    <input
                      type={showPwd ? 'text' : 'password'}
                      value={form.password}
                      onChange={e => set('password', e.target.value)}
                      placeholder={isEdit ? 'Current password loaded' : 'Min. 6 characters'}
                      style={{ paddingRight: '42px', width: '100%', boxSizing: 'border-box' }}
                    />
                    <PwdToggleBtn show={showPwd} onToggle={() => setShowPwd(s => !s)} />
                  </div>
                  <span style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px', display: 'block' }}>
                    {isEdit ? 'Current password load hua hai — aankhon ke button se dekho' : 'Employee login ke liye password'}
                  </span>
                </div>
