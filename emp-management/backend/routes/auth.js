...
router.post('/login', async (req, res) => {
  try {
    ...
    // 1. Pehle User collection check karo (admin / hr)
    let user = await User.findOne({ email });
    ...
    if (user) {
      ...
      // Plain text compare — bcrypt nahi
      const isMatch = await user.comparePassword(password);
      if (!isMatch) return res.status(400).json({ message: 'Invalid credentials' });

      // lastLogin update — findByIdAndUpdate use karo taaki save() hook na chale
    ...
    // 2. Employee collection check karo
    const employee = await Employee.findOne({ email }).populate('department', 'name code');
    if (!employee) {
      return res.status(400).json({ message: 'Invalid email or password' });
    }
    ...
    const isEmpMatch = await employee.comparePassword(password);
    if (!isEmpMatch) return res.status(400).json({ message: 'Invalid email or password' });
...
// ── GET /api/auth/me ────────────────────────────────────────
// Password include kiya — edit form mein dikhne ke liye
router.get('/me', auth, async (req, res) => {
  try {
    if (req.user.type === 'employee') {
      // Employee ke liye password nahi chahiye frontend pe
      const employee = await Employee.findById(req.user.id)
        .populate('department', 'name code');
      if (!employee) return res.status(404).json({ message: 'Employee not found' });
    ...
    // HR / Admin — password bhi bhejo (edit form ke liye)
